...
export function Feedbacks() {
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <h2 className="mb-8 text-center text-4xl font-semibold tracking-tight text-gray-900 sm:text-5xl md:mb-12">
        来访者反馈
      </h2>
...
function TestimonialCard({ name, msg }: feedBack) {
  return (
    <Card className="relative mb-8 w-full max-w-sm break-inside-avoid border-2 bg-gray-50 shadow-none">
      <Quote className="text-foreground/50 absolute top-3 right-2 h-8 w-8 stroke-[2px]" />
      <CardHeader>
        <div className="flex items-center gap-3">
          ...
          */}
          <div className="flex flex-col gap-1">
            <span className="text-lg leading-none font-semibold"> {name} </span>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        <p className="text-muted-foreground text-base">{msg}</p>
      </CardContent>
    </Card>
  )
}
...
const msgs: feedBack[] = [
  ...
  {
    name: '反客为主',
    msg: '同闺蜜参加了两次团体沙盘，原本只是怕她一人尴尬才来陪玩，结果反倒发现了自己内心的真实和缺失……',
  },
  {
    name: '意料之外的轻松',
    msg: '和同性朋友来这里之前，以为沙盘只是小孩和异性朋友间的游戏且气氛沉重，没想到它似乎能让我回想起我的童年、我的初心，清爽清晰的感觉呐。',
  },
]
